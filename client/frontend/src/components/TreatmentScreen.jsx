...
const TreatmentInstructionsDisplay = ({
  caseType,
  degree,
  degrees,
  currentCount,
  ...
  const fetchInstructions = useCallback(
    async (type, deg, degreesParam, count) => {
      ...
      try {
        ...
        }/treatment?case_type=${encodeURIComponent(type)}&count=${count}`;
    ...
    <div>
      {instructionData.map((item, index) => {
        ...
        if (currentCount === 0) {
          ...
          contentToDisplay = (
            <p>{item.description || "No short instruction available."}</p>
          );
        } else if (currentCount === 1) {
          speakableText = item.description;
          contentToDisplay = (
            <p>{item.description || "No detailed instruction available."}</p>
          );
...
export default function TreatmentScreen() {
  ...
  return (
    ...
    >
      ...
      {!initialCaseType && (
        <p style={{ color: "gray" }}>
          Treatment instructions cannot be provided as no diagnosis was made.
          Please return to the chat and describe the emergency.
        </p>
      )}
      ...
      <hr style={{ margin: "2rem 0" }} />
      <button onClick={handleBackToChat}>Back to Chat</button>
      <button onClick={newChat} style={{ marginLeft: "1rem" }}>
        Start New Chat
      </button>
    </div>
  );
